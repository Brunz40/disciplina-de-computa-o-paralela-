...
int main(int argc, char **argv) {
    ...
    float* resultado_produto=malloc((N_recebidos)*sizeof(float));

    for(int i=0; i<N_recebidos; i++) {
        resultado_soma[i]=buffer_destino_A[i]+buffer_destino_B[i];
        resultado_produto[i]=buffer_destino_A[i]*buffer_destino_B[i];
    }
    ...
    if (rank==cabeça) {
        printf("tempo decorrido:%lf\ncarga total:%d\nNúmero de processadores utilizados:%d",fim-inicio,carga,N_tarefas);
